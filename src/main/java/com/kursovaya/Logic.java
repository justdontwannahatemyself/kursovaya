...
public class Logic {
    ...
    public String toString() {
        ...
        for (int i = 0; i < amount; i++) {
            if (i == 0 && result[0][0] > 0) {
                output.append(" ").append(Precision.round(result[0][0], 4)).append("x^").append(amount - 1);
            } else if (i != amount - 1) {
                if (result[i][0] > 0) {
                    output.append(" + ").append(Precision.round(result[i][0], 4)).append("x^").append(amount - i - 1);
                } else {
                    output.append(Precision.round(result[i][0], 4)).append("x^").append(amount - i - 1);
                }
            } else {
                if (result[i][0] > 0) {
                    output.append(" + ").append(Precision.round(result[i][0], 4));
                } else {
                    output.append(Precision.round(result[i][0], 4));
                }
            }
